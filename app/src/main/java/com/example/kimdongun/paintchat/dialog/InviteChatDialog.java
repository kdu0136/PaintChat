...
public class InviteChatDialog extends Dialog implements View.OnClickListener, AdapterView.OnItemClickListener{
     ...
     **********************************/
    public InviteChatDialog(Context context, Client client) {
        super(context);
    ...
    public void createChatRoom(){
        ...
        try {
            ...
            keyJson.put("key", roomKey); //채팅방에 입장 할 유저 아이디
            jsonArr.put(keyJson); //roomKey를 JsonArray에 추가
        ...
        //서버로 전송
        if(((ChatRoomActivity)context_).isLiveBinder)
            ((ChatRoomActivity)context_).socketService_.sendMessage(jsonStr);
